...
def test_standard_case():
    ...
    assert round(project_calories(weight=90, height=1.90, sex=1, age=30, pal=1.2, target_weight=25, number_of_days=40), 2) == 674.77, "Incorrect daily caloric intake calculation!"
...
def test_return_graph():
    ...
    graph = project_calories(weight=68, height=1.83, sex=2, age=27, pal=1.6, target_weight=65, number_of_days=30, return_graph=True)
    ...
    assert graph.layout.title.text == 'Projected Weight Loss', "Plotly Figure title not properly set."
    assert graph.data[0].mode == 'lines', "Plotly Figure title not properly set."
...
def test_negative_or_zero_inputs():
    ...
    with pytest.raises(ValueError):
        project_calories(weight=63, height=1.83, sex=3, age=27, pal=1.6, target_weight=65, number_of_days=30)
    with pytest.raises(ValueError):
        project_calories(weight=63, height=1.83, sex=2, age=0, pal=1.6, target_weight=65, number_of_days=30)
    with pytest.raises(ValueError):
        project_calories(weight=63, height=1.83, sex=2, age=27, pal=0, target_weight=65, number_of_days=30)
    with pytest.raises(ValueError):
        project_calories(weight=63, height=1.83, sex=2, age=27, pal=1.6, target_weight=-65, number_of_days=30)
    with pytest.raises(ValueError):
